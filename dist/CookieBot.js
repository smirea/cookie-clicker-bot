...
class $fe57486f6f15e392$export$2e2bcd8739ae039 {
    ...
    buyTimer() {
        ...
        const run = ()=>{
            ...
            if (threshold.wait) {
                this.log(`🟡 Waiting to buy to threshold for ${threshold.obj.name} - ${Math.round(threshold.nextPrice / 1000000)} M🍪`);
                return;
            }
            if (santa.buy) {
                this.buy({
    ...
    printLog({ buildings: buildings  }) {
        ...
        for (const { time: time , msg: msg , count: count  } of this.logMessages.slice(-1 * this.options.showLogs))console.log('%c[%s]%c %s %c%s', 'color:gray', new Date(time).toISOString().slice(11, 19), 'color:white', msg, 'color:gray', count > 1 ? `✕ ${count}` : '');
    }
}
